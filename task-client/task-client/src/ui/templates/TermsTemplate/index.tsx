...
import {useTermsHook} from '@/hooks/use-terms-hook';
...
const TermsTemplate: React.FC = () => {
  const { terms, isLoading, error } = useTermsHook();
  ...
  useEffect(() => {
    if (terms?.content) {
      // 提取目录
      const toc = extractTableOfContents(terms.content);
      setTableOfContents(toc);

      // 直接使用原始内容，不添加HTML标签，而是依靠Markdown渲染器的特性
      if (searchQuery.trim() === '') {
        setFilteredContent(terms.content);
      } else {
        // 搜索实现，高亮匹配的内容
        const regex = new RegExp(`(${searchQuery})`, 'gi');
        const highlighted = terms.content.replace(regex, '**$1**');
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f5f5f7] dark:bg-[#1c1c1e]">
        ...
        <div className="apple-card bg-white dark:bg-[#2c2c2e] p-8 max-w-md text-center">
          ...
          <p className="text-gray-800 dark:text-gray-200 mb-4 font-medium">加载服务条款时出错</p>
            ...
            <div className="apple-card bg-white/95 dark:bg-[#2c2c2e]/95 apple-blur-bg rounded-2xl overflow-hidden">
              <div className="p-8 md:p-10 lg:p-12">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10">
                  <h1 className="text-3xl font-semibold text-gray-900 dark:text-white">服务条款</h1>
                ...
                <div className="prose prose-lg prose-slate dark:prose-invert max-w-none lg:max-w-4xl mx-auto">
                  {filteredContent ? (
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-center py-12">没有服务条款内容可显示</p>
                  )}
                </div>
                ...
                <div className="mt-12 flex flex-col sm:flex-row items-center gap-4 justify-between border-t border-gray-200 dark:border-gray-700 pt-8">
                  ...
                  <Link href="/register" className="apple-button w-full sm:w-auto inline-flex justify-center items-center px-8 py-3 text-white bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 transition-all duration-200">
                    注册账号
                  </Link>
                </div>
              </div>
