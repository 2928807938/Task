...
| 服务 | 端口 | 访问地址 |
|------|------|----------|
| 前端 | 3000 | http://服务器IP:3000 或你的前端域名 |
| 后端 | 8080 | http://服务器IP:8080 |
...
表示的是 **服务器机器内部监听地址**，不是你本地电脑浏览器应该直接填写的地址。

如果你的服务器公网 IP 是 `38.76.217.249`，那么：

- 前端对外访问地址应为：`http://38.76.217.249:3000`
- 后端对外访问地址应为：`http://38.76.217.249:8080`

如果你已经给前端配置了域名，例如：

- `https://task.airaphe.com`

那么前端页面应从该域名访问；前端里的接口请求地址也必须指向**实际可到达的后端地址**，例如：

- `http://38.76.217.249:8080`
- 或由 Nginx / 网关反向代理后的统一域名接口地址

注意：如果页面是通过 `https://task.airaphe.com` 打开的，但接口却请求到一个没有正确代理到 `8080` 的地址，后端日志里就会表现为“没有收到请求”。
